...
```{r}
plot_model(fit,
           type = "re",            # efectos aleatorios
           sort.est = "sort.all",  # ordena todos los niveles
           grid = FALSE,
           show.values = FALSE,    # evita los números solapados
           show.p = FALSE,         # no muestra p-values
           value.offset = 0.3,     # (si activas valores)
           title = "Random effects of (Intercept)",
           vline.color = "gray")   # línea vertical de referencia
```
...
```
```{r results='asis'}
htmlreg(list(Model4.8, Model4.10, Model4.11))
```
